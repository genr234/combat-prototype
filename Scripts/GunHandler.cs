        ...
        private void Shoot()
        {
            ...
            foreach (var hit in hits)
            {
                ...
                if (enemy != null && !hitEnemies.Contains(enemy))
                {
                    ...
                    // Apply knockback
                    if (currentWeapon != null && currentWeapon.knockbackForce > 0)
                    {
                    ...
                    // Hit stop effect
                    if (currentWeapon != null && currentWeapon.hitStopDuration > 0)
                    {
            ...
            // Handle explosion
            if (currentWeapon != null && currentWeapon.explosionRadius > 0 && didHit)
            {
                HandleExplosion(hitPoint);
            }

            // Play visual effects
            if (shootingEffects != null && currentWeapon != null)
            {
                shootingEffects.PlayAllShootEffects(currentWeapon, hitPoint, hitNormal, didHit);
            }

            // Start cooldown
            var cooldown = currentWeapon != null ? currentWeapon.cooldown : Cooldown;
            onCooldown = true;
